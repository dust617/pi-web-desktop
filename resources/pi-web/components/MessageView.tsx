...
function BlockView({ block, toolResults, isStreaming, streamingDuration, toolCallDurations, cwd, onOpenFile, sessionId, entryId, blockIndex }: { block: AssistantContentBlock; toolResults?: Map<string, ToolResultMessage>; isStreaming?: boolean; streamingDuration?: number; toolCallDurations?: Map<string, number>; cwd?: string; onOpenFile?: (filePath: string) => void; sessionId?: string; entryId?: string; blockIndex: number }) {
  ...
  if (block.type === "toolCall") {
    ...
    // Image preview from tool-result details (e.g., imagegen) — rendered as
    // an independent element AFTER the tool-call block so it is always visible
    // without expanding the tool call and does not visually nest inside it.
    const details = result && !result.isError
      ? (result as ToolResultMessage & { details?: { path?: string; latestPath?: string } }).details
      : undefined;
    const imgPath = details?.path ?? details?.latestPath;
    const imgSrc = imgPath && /\.(png|jpg|jpeg|gif|webp|bmp)$/i.test(imgPath)
      ? `/api/files/${encodeFilePathForApi(imgPath)}?type=read`
      : null;
    return (
      <>
        <ToolCallBlock block={tc} result={result} duration={duration} />
        {imgSrc && (
          <div style={{ margin: "4px 0 8px", borderRadius: 7, overflow: "hidden" }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={imgSrc}
              alt="Generated image"
              style={{ maxWidth: "100%", maxHeight: 400, borderRadius: 7, objectFit: "contain", display: "block", border: "1px solid var(--border)" }}
            />
          </div>
        )}
      </>
...
function PairedResult({ text, isEmpty, isError }: {
  ...
  // Detect image file paths in tool result text (e.g., from imagegen tool)
  // Detect image file paths in tool result text (e.g., from imagegen tool).
  // Supports both forward-slash (D:/path) and backslash (D:\path) Windows paths,
  // plus Unix paths (/path). Also matches markdown image syntax ![alt](path).
  const mdImageMatch = text?.match(/!\[[^\]]*\]\(([^)]+\.(?:png|jpg|jpeg|gif|webp|bmp))\)/i);
  const pathMatch = text?.match(/([A-Z]:[\\/][^\s)]+\.(?:png|jpg|jpeg|gif|webp|bmp)|\/[^\s)]+\.(?:png|jpg|jpeg|gif|webp|bmp))/i);
  const imagePath = mdImageMatch?.[1] ?? pathMatch?.[1] ?? null;
      ...
      {imagePath && (
        <div style={{ padding: "8px 10px", borderBottom: "1px solid var(--border)" }}>
          ...
          <img
            src={`file://${imagePath}`}
            alt="Generated image"
            style={{ maxWidth: "100%", maxHeight: 400, borderRadius: 6, objectFit: "contain", display: "block", border: "1px solid var(--border)" }}
          />
        </div>
      )}
      <pre
