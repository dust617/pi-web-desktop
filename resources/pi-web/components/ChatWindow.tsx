...
// Detect an assistant message whose tool-call result carries an image file
// path in `details` (e.g. imagegen). These should render outside the folded
// ProcessDetailsGroup so the preview is always visible without expanding.
function hasImageToolResult(message: AgentMessage, toolResults: Map<string, ToolResultMessage>): boolean {
  if (message.role !== "assistant") return false;
  for (const block of (message as AssistantMessage).content ?? []) {
    if (block.type === "toolCall") {
      const tc = block as { toolCallId?: string };
      const result = tc.toolCallId ? toolResults.get(tc.toolCallId) : undefined;
      if (result && !result.isError) {
        const details = (result as ToolResultMessage & { details?: { path?: string; latestPath?: string } }).details;
        const p = details?.path ?? details?.latestPath;
        if (p && /\.(png|jpg|jpeg|gif|webp|bmp)$/i.test(p)) return true;
      }
    }
  }
  return false;
          ...
          <div style={{ padding: `0 ${CHAT_COLUMN_PADDING}px` }}>
            ...
            {(() => {
              ...
              for (let idx = 0; idx < messages.length;) {
                ...
                // Split out process messages that carry an imagegen-style result
                // so they render unfolded beside the final answer instead of being
                // buried inside the collapsed ProcessDetailsGroup.
                const imageProcessIndices = visibleProcessIndices.filter((processIdx) => hasImageToolResult(messages[processIdx], toolResultsMap));
                const foldedProcessIndices = visibleProcessIndices.filter((processIdx) => !hasImageToolResult(messages[processIdx], toolResultsMap));
                ...
                const processCount = foldedProcessIndices.length + (finalProcessMessage ? 1 : 0);
                if (processCount > 0) {
                  const processRefIdx = foldedProcessIndices
                  ...
                  const processGroup = (
                    <ProcessDetailsGroup
                      ...
                      toolCallCount={countToolCalls(messages, foldedProcessIndices) + countToolCallBlocks(finalSplit.processBlocks)}
                    >
                      {foldedProcessIndices.map((processIdx) => renderMessage(processIdx, { attachRef: false, keyPrefix: "process" }))}
                ...
                // Render imagegen-style process messages unfolded so their
                // previews are always visible without expanding a group.
                for (const processIdx of imageProcessIndices) {
                  rendered.push(renderMessage(processIdx, { keyPrefix: "process-image" }));
                }
