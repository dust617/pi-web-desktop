...
const fallbackI18nContext: I18nContextValue = {
  ...
  setLocale: () => {},
  t: (key, params) => translateMessage(defaultLocale, key, getMessages(), params),
  supportedLocales: getSupportedLocales().map((id) => getLocalePlugin(id)).filter((plugin): plugin is LocalePlugin => Boolean(plugin)),
};
 ...
 * @returns 若组件树未包裹 Provider，则使用英文回退文案，避免独立预览与测试崩溃。
 */
