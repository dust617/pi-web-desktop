...
function getActiveBranchEntries(entries: SessionEntry[], leafId?: string | null): SessionEntry[] {
  if (leafId === null || entries.length === 0) return [];

  const byId = new Map<string, SessionEntry>();
  for (const entry of entries) byId.set(entry.id, entry);

  // SessionManager uses the current leaf when one is supplied. For a file-only
  // read without a leaf, the last entry is the same fallback used by Pi's
  // session reader for the active branch.
  let currentId = leafId ?? entries.at(-1)?.id;
...
/**
 * Return the last explicit model selection on the active branch.
 *
 * Pi's generic session-state reducer falls back to the provider/model recorded
 * on the latest assistant message. A router intentionally records the concrete
 * backend model there (e.g. gpt-5.6), which must not replace the user's
 * persistent selection of pi-router/auto in the model picker.
 */
function getExplicitModelSelection(entries: SessionEntry[], leafId?: string | null): SessionContext["model"] {
  let selected: SessionContext["model"] = null;
  for (const entry of getActiveBranchEntries(entries, leafId)) {
...
export function buildSessionContext(
  ...
  return {
    ...
    // Prefer an explicit selection over the concrete model recorded in an
    // assistant message. This keeps pi-router/auto visible as the selected
    // mode while the actual backend model remains visible in the response.
    model: getExplicitModelSelection(entries, leafId) ?? piCtx.model,
  };
}
