...
test("keeps an explicit router selection instead of showing the concrete backend model", () => {
  const entries = [
    {
      type: "model_change",
      id: "m1",
    ...
    userEntry("u1", "m1", "route this task"),
    assistantEntry("a1", "u1", "done"),
  ];
  entries[2].message.provider = "openai-codex";
  entries[2].message.model = "gpt-5.6-luna";

  const context = buildSessionContext(entries);

  assert.deepEqual(context.model, { provider: "pi-router", modelId: "auto" });
});
