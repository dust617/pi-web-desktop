...
test("discovery route uses the saved provider for both endpoint and authentication", async () => {
  const source = await readFile(new URL("../app/api/models-config/discover/route.ts", import.meta.url), "utf8");
  assert.match(source, /const provider = getSavedProvider\(providerName\)/);
  assert.match(source, /resolveModelDiscoveryAuth\(providerName, provider\)/);
  assert.doesNotMatch(source, /body\.provider(?!Name)/);
});
