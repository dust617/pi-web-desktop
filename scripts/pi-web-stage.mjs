...
/**
 * Extract test files that contain failing tests from `node --test` output
 * (lines shaped like `test at lib\project-trust.test.mjs:24:1`).
 */
function collectFailedTestFiles(output) {
  const files = new Set();
  const lines = output.split(/\r?\n/);
  let inFailing = false;
  for (const line of lines) {
    if (/failing tests:/i.test(line)) {
      inFailing = true;
      continue;
    }
    if (!inFailing) continue;
    const m = line.match(/^test at (.+):\d+:\d+\s*$/);
...
async function main() {
  ...
  log("  NOTE: On Windows, npm ci may OOM. If it fails, run manually:");
  log(`    cd ${stageDir}`);
  log(`    set NODE_OPTIONS=--max-old-space-size=8192 && npm ci`);
  try {
    run("npm ci", { cwd: stageDir });
    log("NPM CI completed");
  } catch (err) {
    log("npm ci failed. Staging directory preserved for manual retry.");
    log(`  cd ${stageDir}`);
    log(`  set NODE_OPTIONS=--max-old-space-size=8192 && npm ci`);
    rmSync(tmpClone, { recursive: true, force: true });
...
`;
  ...
  try {
    ...
    run("npm run build", { cwd: stageDir, env: buildEnv });
  ...
  // Verify .next static assets are consistent with the server build (same
  // BUILD_ID). A fresh `next build` always emits server/ and static/ in
  // lockstep, but we assert it so a partially-failed or half-copied build can
  // never ship a blank-window trap downstream (HTML references chunk hashes
  // that 404 when static/ is stale).
  const staticDir = join(stageDir, ".next", "static");
  if (!existsSync(join(staticDir, buildId))) {
    rmSync(stageDir, { recursive: true, force: true });
    rmSync(tmpClone, { recursive: true, force: true });
    fail(`.next/static has no <BUILD_ID> subdir (${buildId}) — build artifacts inconsistent`);
  }
  const stagedChunks = join(staticDir, "chunks");
  if (!existsSync(stagedChunks)) {
    rmSync(stageDir, { recursive: true, force: true });
    rmSync(tmpClone, { recursive: true, force: true });
    fail(".next/static/chunks directory missing — static assets incomplete");
  }
  const stagedJsCount = readdirSync(stagedChunks).filter((f) => f.endsWith(".js")).length;
  if (stagedJsCount < 5) {
    rmSync(stageDir, { recursive: true, force: true });
    rmSync(tmpClone, { recursive: true, force: true });
    fail(`.next/static/chunks has only ${stagedJsCount} .js files — static assets incomplete`);
  }
  log(`  static chunks: ${stagedJsCount} .js (consistent with BUILD_ID ${buildId})`);
  ...
  try {
    ...
    if (testResult.status !== 0) {
      ...
      const knownFailures = manifest.knownTestFailures ?? [];
      const failedFiles = collectFailedTestFiles(output);
      const unexpected = [...failedFiles].filter(
        (file) =>
          !knownFailures.some((k) => k.file === file && output.includes(k.test))
      );
      if (unexpected.length > 0) {
        rmSync(stageDir, { recursive: true, force: true });
        rmSync(tmpClone, { recursive: true, force: true });
        fail(
          `Staged pi-web tests failed with unexpected failures: ${unexpected.join(", ")}.\n` +
            `Known environment failures (from manifest): ${knownFailures
              .map((k) => `${k.file} :: ${k.test}`)
              .join(" | ")}`
        );
      }
      log(
        `Staged pi-web tests: ${failedFiles.size} failing file(s) match known environment failures: ${[
          ...failedFiles,
        ].join(", ")}`
      );
    }
