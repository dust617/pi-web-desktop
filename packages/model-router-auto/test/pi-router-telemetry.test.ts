...
test("Pi Router companion only creates paired telemetry after a completed turn", () => {
  const records = completedRouteRecords(
    ...
    3,
    {
      ...
      stopReason: "stop",
      errorMessage: "synthetic provider detail that must not persist",
    },
    250,
  );
  ...
  assert.equal(records.outcome.errorMessage, undefined, "raw provider error text must not enter telemetry");
