...
function init(): void {
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      e.preventDefault();
      // Split the drop: non-image files get their real paths inserted into the
      // textarea as "[外部附件 - 文件] <path>" text; image files are left for
      // the pi-web frontend (React onDrop -> processImageFiles), which builds
      // real image-attachment previews.
      //
      // We must NOT stopImmediatePropagation here. React 18 delegates onDrop to
      // a root-container bubble listener (a descendant of document); calling
      // stopImmediatePropagation in this capture-phase document listener would
      // halt propagation before React's handler ever fires, so useDragDrop's
      // handleDrop would never run -> isDragOver would never reset and the
      // drop-zone overlay would stay stuck on screen. This was the root cause
      // of mixed image+file drops "freezing" on a transparent overlay. React's
      // processImageFiles only ever handles image/ files, so letting it fire
      // for non-image drops is a harmless no-op (it just clears isDragOver).
      const nonImageFiles = files.filter((f) => !f.type.startsWith("image/"));
      if (nonImageFiles.length > 0) handleDrop(nonImageFiles);
    },
    { capture: true }
  );
