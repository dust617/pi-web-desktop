...
/** Verify .next is internally consistent (server + static share one BUILD_ID).
 *  A half-copied .next — e.g. an incremental file-sync that refreshed server/
 *  but left an older static/ in place — lets `next start` boot fine and serve a
 *  valid HTML shell, yet every JS chunk the HTML references 404s (the file
 *  hashes belong to the old build), so the Electron window renders blank.
 *  Catching this at startup turns a silent blank window into a clear error. */
function verifyNextConsistency(piWebDir: string): void {
  const nextDir = path.join(piWebDir, ".next");
  if (!fs.existsSync(nextDir)) {
    throw new Error(`.next 目录缺失：${nextDir}`);
  }
  const buildIdPath = path.join(nextDir, "BUILD_ID");
  if (!fs.existsSync(buildIdPath)) {
    throw new Error(".next/BUILD_ID 缺失（构建产物不完整）");
  }
  const buildId = fs.readFileSync(buildIdPath, "utf8").trim();
  if (!buildId) throw new Error(".next/BUILD_ID 为空");
  if (!fs.existsSync(path.join(nextDir, "required-server-files.json"))) {
    throw new Error(".next/required-server-files.json 缺失（next start 无法启动）");
  }
  if (!fs.existsSync(path.join(nextDir, "server"))) {
    throw new Error(".next/server 目录缺失（服务端产物不完整）");
  }
  const staticDir = path.join(nextDir, "static");
  if (!fs.existsSync(staticDir)) {
    throw new Error(".next/static 目录缺失（前端静态资源丢失，页面将空白）");
  }
  // Core consistency check: `next build` writes a <BUILD_ID> subdir under
  // static/. If static/ instead holds a different BUILD_ID subdir (stale from
  // an incremental sync), the server-rendered HTML references chunk hashes
  // that no longer exist on disk -> 404 -> blank window.
  if (!fs.existsSync(path.join(staticDir, buildId))) {
    throw new Error(
      `.next 构建产物不一致：BUILD_ID=${buildId}，但 static 下无同名子目录` +
      `（疑似增量同步残留了旧构建的 static，会导致页面空白）。` +
      `请从完整一致的 staged 产物整体覆盖 .next，勿增量覆盖。`
    );
  }
  const chunksDir = path.join(staticDir, "chunks");
  if (!fs.existsSync(chunksDir)) {
    throw new Error(".next/static/chunks 目录缺失");
  }
  const jsChunks = fs.readdirSync(chunksDir).filter((f) => f.endsWith(".js"));
  if (jsChunks.length < 5) {
    throw new Error(
      `.next/static/chunks 异常：仅 ${jsChunks.length} 个 .js（预期数十个），static 可能不完整`
    );
  }
}
...
export class PiWebRuntime {
  ...
  private async startInternal(cwd: string | undefined, generation: number): Promise<RuntimeInfo> {
    ...
    // Reject a half-copied .next before spawning next start: an inconsistent
    // build boots and serves HTML, but 404s every JS chunk -> blank window.
    verifyNextConsistency(path.dirname(path.dirname(binPath)));
