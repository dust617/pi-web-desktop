...
function tryApplyStagedPiWebUpgrade(): void {
  ...
  try {
    ...
    // Guard against downgrade: only swap when the staged version is strictly
    // higher. A stale or older staged package must never replace the running
    // version, even if the verification checks otherwise pass.
    if (compareVersions(stagedVersion, currentVer) <= 0) return; // not an upgrade
