...
const MOBILE_USER_IMAGE_MAX_BYTES = 5 * 1024 * 1024; // 用户回显图片上限，与 imagegen 注入口径一致
...
export function slimMobileMessage(message: any): any {
  ...
  const slimBlock = (block: any): any => {
    ...
    if (type === "image") {
      // 用户自己上传的图片（user 角色）保留 base64 回显：SSE 确认/历史重放后移动端
      // 仍能显示，避免“发几条消息后图片变占位”。带大小上限防撑爆移动端 payload。
      // 非 user 角色的 provider/tool 图片按安全策略降级为桌面端占位：
      // 不得把会话内任意 base64 暴露给公网隧道（见 T10 测试）。
      if (role === "user") {
        const rawData = block.data ?? block.source?.data ?? "";
        const mimeType = block.mimeType ?? block.source?.media_type ?? "image/png";
        const bytes = Math.floor(String(rawData).length * 3 / 4);
        if (rawData && bytes <= MOBILE_USER_IMAGE_MAX_BYTES) {
          return { type: "image", data: rawData, mimeType };
        }
