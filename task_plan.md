...
## P8：模型、切换与压缩稳定性核查（已完成）
- [x] 读取 Pi 0.82.1 的 models/settings/compaction/environment 官方文档
- [x] 盘点当前运行模型与全局配置文件
- [x] 对照故障会话，区分超时、传输、上下文阈值与跨模型兼容问题
- [x] 临时备份并最小化调整全局配置；验证后删除含旧凭据的临时备份
- [x] 用配置解析、模型清单、认证探针、阈值检查和 GPT/Qwen smoke 验证
- [x] 记录残余风险与新会话/跨模型操作建议（P8 为在途 v0.2.0 的子阶段，不单独重置共享规划文件）

## P8 遇到的错误
| 错误 | 尝试次数 | 处理 |
|---|---:|---|
| 首次广域 `rg` 命中大量 session 内容并触发输出截断 | 1 | 后续只用结构化脚本提取事件类型、错误与配置元数据，不再扫描消息正文 |
